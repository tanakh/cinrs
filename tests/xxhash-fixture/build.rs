...
//! `scripts/check-xxhash.sh --bench` builds: `target/xxhash/native/`.
//!
//! `#pragma cinrs link "xxhash_gcc"` in `src/lib.rs` names the library; this
...
fn main() {
    ...
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/xxhash/native");
    println!("cargo:rustc-link-search=native={}", dir.display());
    for (on, name) in [(gcc, "gcc"), (clang, "clang")] {
        if on {
            println!("cargo:rerun-if-changed={}/libxxhash_{name}.a", dir.display());
        }
    }
}
