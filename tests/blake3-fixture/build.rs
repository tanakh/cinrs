...
//! `scripts/check-blake3.sh --bench` builds: `target/blake3/native/`.
//!
//! `#pragma cinrs link "blake3_gcc"` in `src/lib.rs` names the library; this
...
fn main() {
    ...
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/blake3/native");
    println!("cargo:rustc-link-search=native={}", dir.display());
    for (on, name) in [(gcc, "gcc"), (clang, "clang")] {
        if on {
            println!("cargo:rerun-if-changed={}/libblake3_{name}.a", dir.display());
        }
    }
}
