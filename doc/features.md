...
`<avxintrin.h>`, `<avx2intrin.h>`, `<x86intrin.h>`, `<popcntintrin.h>` and an
...
does. `__m128`, `__m128i`, `__m128d`, `__m256`, `__m256i` and `__m256d` are
types; `_mm_add_epi32`, `_mm256_loadu_si256`, `_mm_movemask_epi8`,
`_mm_shuffle_epi8`, `_mm_crc32_u32`, `_mm_fmadd_ps`, `_pdep_u64` and eight
hundred and seventy-four others are functions. There is no vector language and
...
the two committed files against each other on every run.
...
are the same set under other names. Eight hundred prototypes are not free to
read: `#include <immintrin.h>` costs about 7 ms of front-end time per
invocation on this machine against 1.4 ms for `<stdio.h>`, `<stdlib.h>` and
`<string.h>` together, and `<emmintrin.h>` — SSE and SSE2, which is what most
code actually uses — about 3 ms. Including the narrower header is worth it in a
crate with many blocks. The macros come with them —
`_MM_SHUFFLE`, `_MM_SHUFFLE2`, `_MM_TRANSPOSE4_PS`, `_MM_HINT_*`,
`_MM_FROUND_*`, `_CMP_*`, `_SIDD_*`, `_MM_ROUND_*` — with the values GCC's
headers give them, which for `_MM_HINT_T0` is 3 rather than the 1 Intel's
documentation prints.

The vector types are ordinary objects of a known size and alignment: sixteen or
thirty-two bytes, aligned to themselves. They may be locals, parameters, return
...
exactly that layout. `sizeof` and `_Alignof` answer 16/16 and 32/32, and a
`struct` holding one is laid out from those. What they are *not* is arithmetic:
...
`adx`, `movbe`, `fxsr`, `xsave`, `rdseed` — is spelled the same. A name that
rustc has not stabilised (`avx512f`, `sse4a`, `rtm`, `gfni`), a processor
...
__AVX2__` takes the other branch here, whatever the machine — the right question
...
`<immintrin.h>`'s eight hundred and eighty-one prototypes add nothing to the
...
* **AVX-512, and the `__m512*` and `__mmask*` types.** Every AVX-512 target
  feature is still unstable in rustc on this crate's minimum supported version
  (1.88), so asking for one would make the build nightly-only.
...
needs `__attribute__((target("avx")))`** (or `avx2`, which implies it). Passing
a `__m256` in a register is only possible with AVX enabled, so rustc refuses the
