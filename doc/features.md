...
Five headers that are not C's are bundled beside them, because a small program
reaches for them and none needs a type with a layout: `<alloca.h>`,
`<sys/types.h>` (the system's `typedef` names, each spelled the way the target's
own library spells it), `<unistd.h>`, `<fcntl.h>` (`open` and the `O_*` flags, at
the target's own values) and `<strings.h>`. The last three are POSIX and say so
with an `#error` when the target is Windows. Anything **with a layout** — `struct
stat`, `sigset_t`, `fd_set`, `pthread_t`, `struct sigaction` — is deliberately
absent: a header that guessed at one of those would corrupt memory rather than
fail to compile. `<signal.h>` is C's, and its signal *numbers* are the
platform's.
