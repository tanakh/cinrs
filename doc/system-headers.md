...
**The bundled set is ISO C, plus what only a compiler can provide.** Every
header in it is one the C standard describes, with two exceptions:
`<alloca.h>`, because `alloca` is generated by this crate rather than called in
any library, and the Intel intrinsics headers — `<immintrin.h>` and its
companions — because `__m128i` and `_mm_add_epi32` are the compiler's too, and
what a call to one becomes is a `core::arch` function rather than a symbol. A
real `<immintrin.h>` is a thicket of `__attribute__((vector_size))` and
`__builtin_ia32_*`; see [SIMD intrinsics](features.md#simd-intrinsics).
POSIX is not bundled:
...
terms of builtins only that compiler has. So do their `immintrin.h` and the rest
of the Intel intrinsics chain, which is `__attribute__((vector_size))` and
`__builtin_ia32_*` from end to end. Every one of them is bundled here anyway, so
leaving them out costs nothing and avoids importing another compiler's idea of
itself — and it is why `#include <immintrin.h>` finds cinrs's copy even under
