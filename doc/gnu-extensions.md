...
| Inline assembly | `asm volatile("…" : "=r"(x) : "r"(y) : "memory")` | occasional (kernels, crypto) | 🟢 supported | Extended and basic asm become `core::arch::asm!` with `options(att_syntax)`, on x86 and x86-64: every operand named, `"r"`/`"q"`/`"x"` as register classes, `"a"` … `"D"` as that register at the operand's width, `"i"` as a folded `const`, a tied `"0"` as `inout`, the `k`/`w`/`b`/`h`/`q` modifiers, register clobbers as `out("…") _`, and `"memory"`/`"cc"` as what `asm!` assumes anyway. Refused by name, each with the rewrite: memory operands (`"m"` — pass `"r"(&x)` and write `(%0)`), `rbx` (which rustc keeps; save it in the template with `xchg`), `%=`, `asm goto`, flag outputs, `"A"`, the x87 and MMX constraints, the range-checked immediates, Intel syntax, and `asm` in a `[[cinrs::safe]]` function. The bundled `<cpuid.h>` is GCC's API on that `xchg` idiom. See [Inline assembly](features.md#inline-assembly). `tests/inline_asm.rs` |
