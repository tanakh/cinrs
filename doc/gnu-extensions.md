...
| Vector extensions | `typedef int v4si __attribute__((vector_size(16)));` | occasional (SIMD) | 🟡 refused (own types); 🟢 on the Intel types | A vector type of the program's own, `vector_size` on a `typedef`, is refused: `core::simd` is unstable, so there is no Rust type with an arbitrary vector's arithmetic. **The Intel intrinsics are the SIMD `cinrs` has** — `#include <immintrin.h>`, `__m128i`, `_mm_add_epi32` — and the refusal names them; `__builtin_ia32_*`, which is GCC's internal name for the same instruction, is refused with the Intel spelling to write instead. What real code uses the extension's operators on, though, is the Intel types, and there they are supported: `a * b + c`, `v * 2.0`, `-v`, `v += w`, the float and double comparisons, `v[i]` as an lvalue lane, and `(__m128d){a, b}` / `= {a, b}` brace initialisers, each lowered to the intrinsic that does the same; integer multiplies, shifts and comparisons are refused with the intrinsic to write. See [SIMD intrinsics](features.md#simd-intrinsics). |
| Intel SIMD intrinsics | `_mm_add_epi32(a, b)`, `__m256i`, `_mm_shuffle_epi8` | common (codecs, hashing, string search, numerics) | 🟢 supported | 6,075 of them, SSE through AVX-512 (F, BW, CD, DQ, VL, VBMI, VBMI2, VNNI, BITALG, VPOPCNTDQ, IFMA, BF16, FP16) with GFNI, VAES, VPCLMULQDQ, AVX-VNNI, AVX-IFMA, F16C, SHA-512, SM3 and SM4, mapped by *name* onto `core::arch::x86_64` (or `core::arch::x86`), whose signatures the bundled headers' prototypes were generated from; a memory operand is `void *` exactly where GCC's headers say so, so an `int *` goes to `_mm512_loadu_si512` without a cast. GCC's operators, subscripts and brace initialisers on the vector types are the intrinsics that do the same (see the row above), and `<mm_malloc.h>`'s `_mm_malloc` and `_mm_free` come with `<xmmintrin.h>`, as they do in GCC. The vector types are opaque objects of a known size and alignment — usable as members, elements, parameters, return values and the members of a punning `union` — and an immediate operand is folded into `core::arch`'s `const` generic. On a target that is not x86 the header is an `#error` naming the reason. See [SIMD intrinsics](features.md#simd-intrinsics) for the whole story, the coverage per instruction set, and what is left out. `tests/simd.rs` |
| `__builtin_cpu_supports`, `__builtin_cpu_init` | common (dispatch on the processor) | 🟢 supported | `__builtin_cpu_supports("avx2")` is `std::is_x86_feature_detected!("avx2")` — the same `cpuid` leaves, answered as an `int` — taking GCC's own names for an instruction set (`bmi` is Rust's `bmi1`, `pclmul` is `pclmulqdq`, `abm` is LZCNT and POPCNT together). It needs `std`, so under `#pragma cinrs no_std` it is a located error, and on a target that is not x86 it is refused with the reason. `__builtin_cpu_init()` is accepted and does nothing: Rust's detection is lazy and needs no initialiser. `__builtin_cpu_is` is **not** here — it names a microarchitecture, which `std_detect` cannot answer — and `__has_builtin` says so, which is what makes a guarded program ask about the instruction instead. |
| Inline assembly | `asm volatile("…" : "=r"(x) : "r"(y) : "memory")` | occasional (kernels, crypto) | 🟢 supported | Extended and basic asm become `core::arch::asm!` with `options(att_syntax)`, on x86 and x86-64: every operand named, `"r"`/`"q"`/`"x"` as register classes, `"a"` … `"D"` as that register at the operand's width, `"b"` through a scratch register swapped with rbx by an `xchg` around the template, `"i"` as a folded `const`, a tied `"0"` as `inout`, the `k`/`w`/`b`/`h`/`q` modifiers, register clobbers as `out("…") _`, and `"memory"`/`"cc"` as what `asm!` assumes anyway. Refused by name, each with the rewrite: memory operands (`"m"` — pass `"r"(&x)` and write `(%0)`), an `rbx` clobber (which rustc keeps; use a `"=b"` operand), `%=`, `asm goto`, flag outputs, `"A"`, the x87 and MMX constraints, the range-checked immediates, Intel syntax, and `asm` in a `[[cinrs::safe]]` function. The bundled `<cpuid.h>` is GCC's API on that `xchg` idiom. See [Inline assembly](features.md#inline-assembly). `tests/inline_asm.rs` |
| `asm` labels on declarations | `int f(void) __asm__("f_impl");` | common (libc shims) | 🟢 supported | On a declaration the unit does not define, `#[link_name = "…"]`; on one it defines, `#[unsafe(export_name = "…")]`. On a local variable — GCC's register variable, `register int x asm("eax")` — it is refused: write the register as a constraint of the `asm` that uses it, `"a"(x)`. |
...
| `vector_size(N)` | occasional | 🟡 refused | See vector extensions: a vector type of the program's own is refused, and the operators work on the Intel types. |
| `target("…")` | common (SIMD dispatch) | 🟢 supported | `#[target_feature(enable = "…")]` on the generated item, which is the same promise: the function may use that instruction set, and whether the processor has it is the program's business — `__builtin_cpu_supports` is how to ask. GCC's spelling of a name is translated to LLVM's (`bmi` → `bmi1`, `pclmul` → `pclmulqdq`, `cx16` → `cmpxchg16b`, `rdrnd` → `rdrand`, `sse4` → both halves of SSE4, `abm` → `lzcnt` and `popcnt`); several names in one string (`target("sse4.2,popcnt")`) and several attributes on one declaration both accumulate, as GCC's do. **Refused** rather than ignored, because an ignored `target` is a function compiled for the baseline and a program that faults: a name rustc has not stabilised (`sse4a`, `rtm`, `tbm`; the AVX-512 names, `gfni`, `vaes` and the rest are stable and accepted), `mmx` (no `__m64` in `core::arch`), a processor (`target("arch=haswell")`, `tune=`), and turning an instruction set off (`target("no-avx")`, which `#[target_feature]` cannot express). On a `[[cinrs::safe]]` function it is refused with the reason: `#[target_feature]` makes a function unsafe to *call*, which is the opposite of what safe promises. `tests/ui/simd_target_attribute.rs` |
| `target_clones("…")` | rare | 🟢 accepted | Ignored, so the function is compiled once, for the baseline. GCC's version resolves at load time through an `ifunc`, which has no stable Rust counterpart — write the `__builtin_cpu_supports` branch by hand. |
| `nonnull`, `returns_nonnull`, `malloc`, `pure`, `const`, `leaf`, `nothrow`, `access(…)`, `alloc_size`, `alloc_align`, `sentinel`, `returns_twice`, `no_sanitize`, `noclone`, `noipa`, `optimize(…)`, `error(…)`, `warning(…)`, `designated_init`, `artificial`, `gnu_inline`, `externally_visible` | common in library headers, rare in bodies | 🟢 accepted | Optimisation and diagnostic hints only. |
...
| `__has_attribute`, `__has_builtin`, `__has_feature`, `__has_extension`, `__has_c_attribute` | common (portability) | 🟢 supported | Answered from [`crate::gnu`](../crates/cinrs-core/src/gnu.rs)'s tables, so the answer is true of *this* implementation: `__has_attribute(packed)`, `__has_attribute(cleanup)` and `__has_attribute(target)` are 1, `__has_attribute(vector_size)` is 0, `__has_builtin(__builtin_cpu_supports)` is 1 and `__has_builtin(__builtin_cpu_is)` is 0. |
