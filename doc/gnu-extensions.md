...
| `target("…")` | common (SIMD dispatch) | 🟢 supported | `#[target_feature(enable = "…")]` on the generated item, which is the same promise: the function may use that instruction set, and whether the processor has it is the program's business — `__builtin_cpu_supports` is how to ask. GCC's spelling of a name is translated to LLVM's (`bmi` → `bmi1`, `pclmul` → `pclmulqdq`, `cx16` → `cmpxchg16b`, `rdrnd` → `rdrand`, `sse4` → both halves of SSE4, `abm` → `lzcnt` and `popcnt`); several names in one string (`target("sse4.2,popcnt")`) and several attributes on one declaration both accumulate, as GCC's do. **Refused** rather than ignored, because an ignored `target` is a function compiled for the baseline and a program that faults: a name rustc has not stabilised (everything AVX-512, `sse4a`, `rtm`, `tbm`, `gfni`), `mmx` (no `__m64` in `core::arch`), a processor (`target("arch=haswell")`, `tune=`), and turning an instruction set off (`target("no-avx")`, which `#[target_feature]` cannot express). On a `[[cinrs::safe]]` function it is refused with the reason: `#[target_feature]` makes a function unsafe to *call*, which is the opposite of what safe promises. `tests/ui/simd_target_attribute.rs` |
