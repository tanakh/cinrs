...
[`doc/gnu-extensions.md`](gnu-extensions.md) is the catalogue for both, and
[`doc/system-headers.md`](system-headers.md) is what happens when the
platform's own headers are read instead of the bundled ones.
...
Nor does the model reach a type whose *layout* is the library's own rather than
the ABI's: `struct stat`, `DIR`, `pthread_mutex_t`, `regex_t`, `struct
utsname`, the real `FILE`. Those are not guessed at — a header that guessed
would corrupt memory rather than fail to compile — and are read from the
machine instead, when a unit asks for it with `#pragma cinrs system_include`.
That is the one place the model steps aside: the platform's headers describe
the *host*, so a cross build gets no default directories at all and must point
`CINRS_SYSTEM_INCLUDE_PATH` at a sysroot. [`doc/system-headers.md`](system-headers.md)
...
| Remove implicit function declaration | N636 | 🟢 Yes | Error from `c99!` up, GNU dialects included. In `c89!` and `gnu89!` a call to an undeclared `f` declares `extern int f();` at file scope from that point on — no prototype, so the arguments get the default argument promotions — and it becomes an `extern` declaration like any other, so `abs(-3)` in a program that includes nothing links against the C library. A later declaration must be compatible with it or it is the ordinary "conflicting types". A C library function GCC has a built-in for (`strcpy`, `strcmp`, `memcpy`, `printf`, …) is declared with the library's prototype instead, as GCC does — also when declared without one (`char *strcpy();`) — so its arguments are converted and its return type is the real one; see [K&R C](features.md#kr-c-implicit-int-and-implicit-declarations). A `__builtin_` name is never declared this way: it belongs to the implementation, and a diagnostic beats a link error. |
...
| Translation phase 2 (line splicing, 5.1.1.2p1) | 🟢 Yes | A backslash-newline is deleted *before* the source is split into tokens, so one may sit in the middle of an identifier: `__LI\<newline>NE__` is the one identifier `__LINE__`, which is what Clang's `drs/dr464.c` and `C99/n590.c` require. A splice needs string-literal input (see [Input forms](features.md#input-forms)), because Rust's own lexer will not hand a line continuation over in raw-token form. A splice inside a *number* or a *punctuator* — `1\<newline>2`, `+\<newline>=` — is still a token boundary, which no real program depends on. |
...
| `<wchar.h>` and `<wctype.h>` | 🟢 Yes | Bundled. `wchar_t` is `int` (from `<stddef.h>`) on every target, which is what the front end gives `L'x'` and `L"…"`; `mbstate_t` is spelled the way each platform's library lays it out. `wcstold` is left out with `long double`; a program's own declaration of it is linked to `wcstod` (see the `long double` row). |
...
| POSIX headers: `<sys/types.h>`, `<unistd.h>`, `<fcntl.h>`, `<strings.h>` | ⚪ N/A (bundled) | Not C, and bundled anyway because a small program reaches for them and none of the four needs a type with a layout: the system `typedef` names (`ssize_t`, `off_t`, `pid_t`, `uid_t`, `mode_t`, `dev_t`, `ino_t`, `nlink_t`, `blksize_t`, `blkcnt_t`, `suseconds_t`, `id_t`, `key_t`, `caddr_t` …), `read`/`write`/`close`/`lseek`/`getpid`/`sleep`/`_exit`, `open` with the `O_*` flags, and the BSD string functions. Every type and every flag value comes from the [target model](#the-target-model), read off glibc, Apple's library and the Microsoft runtime. The last three are POSIX and say so with an `#error` when the target is Windows. `struct stat`, `fd_set`, `sigset_t` and the `pthread_*` types are deliberately absent, for the layout reason above — they come from the machine's own headers instead, with `#pragma cinrs system_include`; see [`doc/system-headers.md`](system-headers.md). |
| The rest of POSIX: `<sys/stat.h>`, `<dirent.h>`, `<pthread.h>`, `<regex.h>`, `<netdb.h>`, `<termios.h>` … | ⚪ N/A (the platform's) | Read from the machine itself, since only it knows the layouts. `#pragma cinrs system_include` puts `/usr/include` and its like on the search path, after the bundled headers or — with `first` — before them. All sixty-seven headers swept go through this front end unchanged in both `gnu11!` and `c11!`, and the programs written against them link and run against the real library; [`doc/system-headers.md`](system-headers.md) is the table and the search rules. |
| `setjmp`/`longjmp` | 🔴 No | They resume a saved machine context, and the state a `longjmp` would return into is the generated Rust's — which the compiler is entitled to assume nothing leaves that way. The bundled `<setjmp.h>` is an `#error` and nothing else; the *call* is refused by name as well (`setjmp`, `_setjmp`, `sigsetjmp`, `__sigsetjmp`, `longjmp`, `_longjmp`, `siglongjmp` and the `__libc_` spellings), because the platform's own `<setjmp.h>` is one [`system_include`](system-headers.md) away and declares them as ordinary functions. *Declaring* them, and declaring a `jmp_buf`, are both fine — half of POSIX pulls the header in. |
| `long double` | 🟡 Partial | Mapped to `double`, which is self-consistent for everything a unit defines — its own functions, its own `va_arg(ap, long double)`. At the boundary with a platform whose `long double` is wider (x87 on x86-64 System V and i386, a 128-bit quad on AArch64 Linux and most 64-bit targets) nothing is silently wrong: a declared-only `strtold`, `wcstold`, `<math.h>` or `<complex.h>` `l` function, or `nexttoward`, is linked to its `double` twin (`#[link_name = "strtod"]`; the table is `sema::long_double::LONG_DOUBLE_TWINS`), and any other declared-only function with a `long double` or `long double *` in its prototype is refused where it is called or its address taken, as is a `long double` or `long double *` passed through a declared-only function's `...` (`printf("%Lf", x)`, `sscanf("%Lf", &x)`) — cast to `double` and use `%f`. On MSVC, 32-bit Arm and Apple arm64 the platform's `long double` is `double` and nothing is refused, but the twins are linked to the sibling there too — the UCRT exports no `powl`, `sinl`, `fabsl`, …, which `<corecrt_math.h>` defines inline over `pow`, `sin`, `fabs`. `tests/ui/long_double_abi.rs` |
| `va_list` as a struct member or file-scope object | 🔴 No | `core::ffi::VaList` carries a lifetime, and only a function signature and a `let` may leave it elided — so a member, a file-scope object and a *return type* are each refused, a `va_list *` in one of those places included. A `va_list *` **parameter or local** is fine, and is what lets a helper advance the caller's list; see [`doc/features.md`](features.md#variadic-functions). |
...
| Benign typedef redefinition | N1360 | 🟢 Yes | 6.7p3: a `typedef` name may be redeclared, in the same scope, to the *same* type. Accepted in every entry point rather than only from C11 on — C89 and C99 forbade it and every compiler took it anyway, and no program is made wrong by the leniency. Two different types under one name is still the constraint violation it always was, and says which declaration it clashes with. This is what lets a unit mix headers: the bundled `<stdint.h>` and the platform's `<sys/types.h>` both spell `size_t`, `int64_t` and `ssize_t`, and every header set repeats its neighbours' typedefs. It is also why the bundled `<sys/types.h>` spells `dev_t` and `ino_t` the way the platform's own headers do — `unsigned long` where that is already 64 bits — since two spellings of the same width are two types. `tests/system_headers.rs` |
...
| TS 18661 integration (`_FloatN`, decimal floating types) | N2314, N2341, N2359, N2546, N2640, N2755, N2931, N2754 | 🟡 Partial | `_Float32` is `float`, `_Float64` and `_Float32x` are `double`, `_Float64x` is `long double` (so `double`, with the platform boundary of `long double` applied), each with `_Complex`; `_Float128`/`__float128` may be named (prototypes, `typedef`s, pointers, `sizeof`, `_Generic`) but no value of it is accepted; `_Float16`, `_Float128x` and the decimal types are refused — no stable Rust types. The `f32`…`f128` literal suffixes are GNU extensions; see [`doc/gnu-extensions.md`](gnu-extensions.md). |
...
| `__has_include` | N2799 | 🟢 Yes | Resolved exactly as `#include` is. GNU's `__has_include_next` too, and it answers the question `#include_next` would: is there one *after* the place this file was found in? |
