...
| Enhanced enumerations (fixed underlying type) | N3030 | 🟡 Partial | The enumeration *is* the type written — its enumerators have it, `sizeof` gives its size, and a tagged one at file scope becomes an alias for it — and every declaration of one tag has to agree about it: a fixed type where an earlier declaration had none, none where an earlier one had a fixed type, and two different fixed types are all constraint violations with a located error. What is not enforced is 6.7.2.2's other rule: a *non-defining* declaration with a fixed underlying type is only allowed standing alone, so `enum E : long x;` and `void f(enum a : long b);` are accepted here and refused by GCC and Clang. |
