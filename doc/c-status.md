...
| The rest of POSIX: `<sys/stat.h>`, `<dirent.h>`, `<pthread.h>`, `<regex.h>`, `<netdb.h>`, `<termios.h>` … | ⚪ N/A (the platform's) | Read from the machine itself, since only it knows the layouts. `#pragma cinrs system_include` puts `/usr/include` and its like on the search path, after the bundled headers or — with `first` — before them. Sixty-six of the sixty-seven headers swept go through this front end unchanged in both `gnu11!` and `c11!` — the exception is `<tgmath.h>` — and the programs written against them link and run against the real library; [`doc/system-headers.md`](system-headers.md) is the table, the search rules and the one gap. |
