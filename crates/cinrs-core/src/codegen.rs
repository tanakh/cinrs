...
//! # Functions that jump
//!
//! Most `goto`s stay here too. [`regions`](crate::regions) has wrapped the
...
//! ```
//!
//! named after the C label — with the number of the label appended when Rust
...
//! stands in.
//!
//! A body sema lowered into a [control-flow graph](crate::cfg) instead —
//! because it holds a jump Rust cannot make at all — is emitted as a state
//! machine over its blocks, with the function's locals defined once at the
//! top. Everything below the statement level is shared: the expressions,
//! places and conversions are generated by exactly the same code in both
//! modes.
//!
...
///
/// A label is not a raw identifier, so a Rust keyword — `loop:` is a perfectly
/// ordinary C label — cannot be one, nor can the five names that cannot even
/// be raw. The labels this module builds itself are kept clear too, so that a
/// `break` in a region never names a loop instead. A C label that is one of
/// them keeps its own name with the label's number appended.
...
/// Every [region](ir::Region) of a body, outermost first, with the C label it
/// is named after.
    ...
    for stmt in stmts {
        match stmt {
            Stmt::Region(region) => {
                ...
                collect_regions(&region.body, out);
            }
...
struct Codegen<'a> {
    ...
    /// The Rust label each [region](ir::Region) of the function being
    /// generated carries: the C label's name, unless Rust cannot spell it as a
    /// label or this module already gives that name to a loop.
...
impl<'a> Codegen<'a> {
    ...
    fn stmt(&mut self, stmt: &Stmt) -> TokenStream {
        match stmt {
            ...
            // The CFG lowering consumes these, and a label the structured mode
            // kept is where a region ends rather than anything of its own: only
            // the statement under it is left.
            Stmt::Label { body, .. } | Stmt::Case { body, .. } => self.stmt(body),
            // A `goto` left in a structured body leaves the region its label
            // stands for; see [`crate::regions`]. Anything else is consumed by
            // the CFG lowering, which is the only mode it is generated in.
            Stmt::Goto { id, range } => {
                let span = self.sp(*range);
    ...
    /// The Rust label a [region](ir::Region) carries.
    fn region_label(&self, id: ir::LabelId, span: Span) -> TokenStream {
        match self.region_names.get(&id) {
            Some(name) => self.label(name, span),
            // Only a `goto` outside every region for its label, which
            // [`crate::regions`] does not leave behind.
            None => self.label(&format!("cinrs_label{}", id.0), span),
        }
    }

    /// A labelled block or a labelled loop, and the statements inside it.
    ///
    /// The two shapes are what an outward `goto` becomes — see
    /// [`crate::regions`]. A loop ends with a `break` of its own so that
    /// falling off the end of the region leaves it; one whose body cannot
    /// reach its end has none, which is what lets a function ending in it need
    /// no `return`.
    fn region(&mut self, region: &ir::Region) -> TokenStream {
        ...
        let body = self.stmts(&region.body);
        match previous {
            Some(kind) => self.region_kinds.insert(region.label, kind),
            None => self.region_kinds.remove(&region.label),
        };
        match region.kind {
    ...
    /// Names the regions of a structured body, after the C labels they stand
    /// for.
    ///
    /// A name that Rust cannot spell as a label — a keyword, or one of the
    /// shapes this module gives a loop or a `switch` — gets the label's number
    /// appended, which no generated label can collide with; one it cannot
    /// spell at all, such as an extended identifier, gives up its own name.
    /// The block and the loop a label with jumps of both kinds gets are named
    /// once: they never overlap, and `break` and `continue` say which is meant.
