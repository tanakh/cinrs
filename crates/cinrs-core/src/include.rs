...
///
/// Every one of them is a header ISO C describes, with four exceptions:
/// `<alloca.h>` is here because `alloca` is generated by this crate rather than
/// called in any library, `<mm_malloc.h>` because it is the compiler's own
/// (GCC's and Clang's `<xmmintrin.h>` include it, and programs count on the
/// `<stdlib.h>` it brings), in portable C over `malloc` because GCC's calls
/// `posix_memalign`, which the Microsoft runtime lacks; the Intel intrinsics
/// headers are here because
/// `_mm_add_epi32` is generated too — as a call to `core::arch` — and
/// `<cpuid.h>` is here because GCC's copy names rbx as an `asm` operand, which
/// rustc refuses, and this one saves and restores it in the template instead.
/// In each case the compiler's copy is not one `cinrs` could use.
