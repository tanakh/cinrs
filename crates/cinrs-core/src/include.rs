...
//!
//! **The bundled set is ISO C and nothing else.** Everything in [`BUNDLED`] is
//! a header the C standard describes — plus `<alloca.h>`, which is there
//! because `alloca` is implemented by this crate rather than by any library.
...
///
/// Every one of them is a header ISO C describes, with one exception:
/// `<alloca.h>` is here because `alloca` is generated by this crate rather than
/// called in any library, so there is nothing for the platform's copy to add.
